...
async function createUser(name, surname, email, password, idNumber, iban, role, isVerified, location) {
    try {
        const user = await User.create({name, surname, email, password, idNumber, iban, role, isVerified, location});
...
async function createArticle(title, body, authorId) {
    try {
        const article = await Article.create({title, body, authorId});
...
async function main() {
    ...
    await createUser('Cansu', 'Tekin', 'eurohack@yahoo.ca', '123456', "11111111111", "TR480006274832592429697373", 'BASIC', false, {longitude: 40, latitude: 29});
    await createUser('Kâni', 'Hayal', 'mcrawfor@comcast.net', '123456', "11111111112", "TR510006257213591611697964", 'TRADER', true, {longitude: 41, latitude: 30});
    await createUser('Ugur', 'Gultekin', 'report@comcast.net', '123456', "11111111113", "TR900006252256359556981742", 'TRADER', false, {longitude: 42, latitude: 31});
    await createUser('Bob', 'Billings', 'hellfire@sbcglobal.net', '123456', "11111111114", "TR810006221368259924941966", 'TRADER', true, {longitude: 43, latitude: 32});
    await createUser('Jim', 'Jones', 'cderoove@hotmail.com', '123456', "11111111115", "TR840006234977194221895499", 'BASIC', true, {longitude: 44, latitude: 33});
    ...
    await createArticle('Low-Cost Investing Can’t Get Any Lower Than Free', 'Charles Schwab said Tuesday that it would allow customers to trade stocks and exchange-traded funds for free, ramping up the intensity of brokers’ fight for the average investor’s dollars.\n' +
        ...
        'Investment firms have long been moving in this direction, with no-fee E.T.F. offerings expanding steadily since they first appeared about a decade ago.', users[0]._id);
    await createArticle("Another Fed Rate Cut Is Expected After Weak Economic Data", 'Top Federal Reserve officials have said they are open-minded about whether additional interest rate cuts will be necessary in 2019, but recent economic data are putting that equanimity to the test.\n' +
        ...
        '“We’ll learn more before the meeting at the end of this month,” he said. “Whether or not one more rate cut at this point is the right decision or not, I think we’re just going to have to go into the meeting and see.”', users[3]._id);
    ...
    await createArticle('Test Book 2', 'Longer-term bonds have been trading at interest rates that are lower than those on short-term securities — what is known as the yield curve inverting. It’s an unusual occurrence that often happens before recessions, and one that could signal that investors have become pessimistic about the economic outlook.\n' +
        ...
        'Based on economic projections released Wednesday, a growing number of Fed officials expect one more reduction this year — in-line with investor and economist expectations.', users[4]._id);
}
